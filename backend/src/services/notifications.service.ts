...
export const createNotification = async (params: CreateNotificationParams) => {
  ...
  if (priority === 'URGENT') {
    try {
      ...
      console.error('Error enviando email de notificación urgente:', error);
    }
  }
  ...
  const notifications = await Promise.all(
    userIds.map(userId =>
      createNotification({
        ...
        message: reason
          ? `La firma del documento "${documentName}" ha sido revertida. Razón: ${reason}`
...
export const cleanExpiredNotifications = async () => {
  ...
  console.log(`🧹 Limpieza de notificaciones expiradas: ${result.count} eliminadas`);
