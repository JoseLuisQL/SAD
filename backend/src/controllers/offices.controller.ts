...
import * as officesService from '../services/offices.service';
import { createOfficeSchema, updateOfficeSchema, uuidSchema } from '../utils/validators';
...
export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await officesService.getAllOffices(filters);
    ...
    res.status(500).json({
      status: 'error',
      message: error instanceof Error ? error.message : 'Error al obtener oficinas'
    });
  }
};
...
export const getById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (error instanceof Error && error.message === 'Oficina no encontrada') {
      res.status(404).json({
        status: 'error',
    ...
    res.status(500).json({
      status: 'error',
      message: error instanceof Error ? error.message : 'Error al obtener oficina'
    });
  }
};
...
export const create = async (req: Request, res: Response): Promise<void> => {
  try {
    const { error, value } = createOfficeSchema.validate(req.body);
    ...
    const office = await officesService.createOffice(value, req);
...
export const update = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { error, value } = updateOfficeSchema.validate(req.body);
    ...
    const office = await officesService.updateOffice(id, value, req);
    ...
    res.status(500).json({
      status: 'error',
      message: error instanceof Error ? error.message : 'Error al actualizar oficina'
    });
  }
};
...
export const deleteOffice = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await officesService.deleteOffice(id, req);

    res.status(204).send();
  } catch (error) {
    if (error instanceof Error) {
      if (error.message === 'Oficina no encontrada') {
        res.status(404).json({
          status: 'error',
          message: error.message
        });
    ...
    res.status(500).json({
      status: 'error',
      message: error instanceof Error ? error.message : 'Error al eliminar oficina'
    });
  }
};
...
export const search = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const offices = await officesService.searchOffices(q);
    ...
    res.status(500).json({
      status: 'error',
      message: error instanceof Error ? error.message : 'Error al buscar oficinas'
    });
  }
};
...
export const getStats = async (_req: Request, res: Response): Promise<void> => {
  try {
    const stats = await officesService.getStats();
...
export const exportCSV = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const data = await officesService.exportToCSV(filters);

    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename=oficinas.csv');
...
export const exportExcel = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const data = await officesService.exportToExcel(filters);
...
export const importCSV = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await officesService.importFromCSV(data, req);
...
export const importExcel = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await officesService.importFromExcel(data, req);
...
export const bulk = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    switch (operation) {
      case 'create':
        result = await officesService.bulkCreate(data, req);
        break;
      case 'update':
        result = await officesService.bulkUpdate(data, req);
        break;
      case 'delete':
        result = await officesService.bulkDelete(data, req);
        break;
      default:
        res.status(400).json({
          status: 'error',
